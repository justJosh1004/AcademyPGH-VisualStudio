...
namespace Battleship
{
    ...
    static string draw_grid(int x, int y)
    {
            ...
            if ((x == ship1[0] && y == ship1[1]) || (x == ship2[0] && y == ship2[1]))//if user hits the ship
            {
                grid[y - 1, x - 1] = "X";//changes the display to show a hit
            */
                //makes the grid
                Console.WriteLine("vY 1 2 3 4 5 6 7 8");
                for (int i = 0; i < 8; i++)
            ...
            {
                grid[y - 1, x - 1] = "M";//changes the display for a miss
